...
| 段階 | 内容 | 次に進む条件 |
|---|---|---|
| 0 | 報酬 v2 の効果を数世代見る(並行して進めてよい) | — |
| 1 | 複数ゲームの同時進行を入れる(Windows のまま) | 1世代の時間が縮み、成績が変わらないこと |
| 2 | コンテナで `nvidia-smi` と torch の GPU 認識を確かめる | GPU が見えること |
| 3 | コンテナで Kev-4B を CUDA グラフなしで動かし、高速化ライブラリの有無で推論と学習の速さを比べる | 1手の推論時間と `s/rec` が明らかに縮むこと |
